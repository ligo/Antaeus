...
public class GenericFastImageSearcher extends AbstractImageSearcher {
    ...
    protected static LinkedHashMap<Integer, byte[]> featureCache = null;
    public static LinkedHashMap<Integer, byte[]> feature = new LinkedHashMap<>();
...
//                        featureCache.put(i, new SearchItem(cachedInstance.getByteArrayRepresentation(), new SimpleResult(-1d, i, d.getValues(OceanDocumentBuilder.FIELD_NAME_IDENTIFIER)[0])));
//                        featureCache.put(i, new SearchItem(i, cachedInstance.getByteArrayRepresentation(), d.getValues(OceanDocumentBuilder.FIELD_NAME_IDENTIFIER)[0]));
...
//        }
    ...
    public ImageSearchHits search(BufferedImage image, IndexReader reader) throws IOException {
        ...
        if (extractorItem.isGlobal()){
            ...
            //double maxDistance = findMultiThreadSimilar(reader, globalFeature);
            if (!useSimilarityScore) {
                searchHits = new SimpleImageSearchHits(this.docs, maxDistance);
                //searchHits = new SimpleImageSearchHits(this.concurrentSkipListSet, maxDistance);
            } else {
                searchHits = new SimpleImageSearchHits(this.docs, maxDistance, useSimilarityScore);
                //searchHits = new SimpleImageSearchHits(this.concurrentSkipListSet, maxDistance, useSimilarityScore);
            }
