...
public class GlobalDocumentBuilder implements DocumentBuilder {
    ...
    private HashingMode hashingMode = HashingMode.LSH;
...
//        else result = new Field[1];
    ...
    public Field[] createDescriptorFields(double[] feature) {
        Field[] result = null;
        docsCreated = true;
        Field field = null;
        Field hash = null;
        ...
        if (hashingEnabled) {
            // TODO: check eventually if there is a more compressed string version of the integers. i.e. the hex string
            if (feature.length <= 3100) {
                int[] hashes;
                if (hashingMode == HashingMode.BitSampling) {
                    hashes = BitSampling.generateHashes(feature);
                    hash = new TextField(OceanFeature.FIELD_NAME_OCENAI_FEATURE + "_hash", SerializationUtils.arrayToString(hashes), Field.Store.YES);
                } else if (hashingMode == HashingMode.LSH) {
                    hashes = LocalitySensitiveHashing.generateHashes(feature);
                    hash = new TextField(OceanFeature.FIELD_NAME_OCENAI_FEATURE + "_hash", SerializationUtils.arrayToString(hashes), Field.Store.YES);
                }
            } else {
                System.err.println("Could not create hashes, feature vector too long: " + feature.length + " (" + OceanFeature.class.getName() + ")");
            }
        }

        if (hash != null) result = new Field[]{field, hash};
        else result = new Field[]{field};
    ...
    public Document createDocument(double[] feature, String identifier) {
        ...
        Document doc = new Document();
        if (identifier != null) {
            doc.add(new StringField(net.semanticmetadata.lire.builders.DocumentBuilder.FIELD_NAME_IDENTIFIER, identifier, Field.Store.YES));
        }
