...
//import net.semanticmetadata.test.imageanalysis.features.global.ACCID;
...
//    }
    ...
    /**
     * WARNING!! This can be used in order to use a different OceanDocumentBuilder than the {@link GlobalDocumentBuilder}, {@link LocalDocumentBuilder} or the {@link SimpleDocumentBuilder}.
     * Every time only one custom OceanDocumentBuilder can be used. At the same time, using the addExtractor methods one can add other builders to be used at the same time. BUT when using
     * a custom OceanDocumentBuilder, sampling can be used for Local Features. This means that, if you want to use a {@link LocalDocumentBuilder} or {@link SimpleDocumentBuilder}, you can use
